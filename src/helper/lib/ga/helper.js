...
export default function (
  ...
  return {
    initIndividualChromosomeFunction () {
      const result = zeros(CHILDREN, VACCINE, NUMBER).toArray()
          ...
          for (let numberIndex = 0; numberIndex < NUMBER; numberIndex++) {
            if (VACCINE_NUMBER[childrenIndex][vaccineIndex][numberIndex]) {
              ...
              if (childOld <= finalTime && childOld >= startTime) {
                ...
                result[childrenIndex][vaccineIndex][numberIndex] = day
              }
            }
          }
        }
    ...
    crossIndividualChromosomeFunction (father, mother) {
      const result = zeros(CHILDREN, VACCINE, NUMBER).toArray()
      for (let childrenIndex = 0; childrenIndex < CHILDREN; childrenIndex++) {
        for (let vaccineIndex = 0; vaccineIndex < VACCINE; vaccineIndex++) {
          for (let numberIndex = 0; numberIndex < NUMBER; numberIndex++) {
            if (VACCINE_NUMBER[childrenIndex][vaccineIndex][numberIndex]) {
              ...
              if (childOld <= finalTime && childOld >= startTime) {
                const r = random()
                if (r < 0.5) {
                  result[childrenIndex][vaccineIndex][numberIndex] = father[childrenIndex][vaccineIndex][numberIndex]
                } else {
                  result[childrenIndex][vaccineIndex][numberIndex] = mother[childrenIndex][vaccineIndex][numberIndex]
                }
              }
            }
          }
